...
public class Extractor {
    ...
    public static void main(String[] args) {
        System.out.println(searchVidPage("Bruno Mars").getMp4Audio());
    }
    ...
    public static String decryptSignature(String sig, String playerURLString) {
        ...
        try {
            ...
            methodName = regBetween("sig\\|\\|", "..", "\\(", playerPage);
            if (debug)
                System.out.println("Signature method name= " + methodName);

            actualMethod = regBetween("", "function " + methodName.replace("$", "\\$") + ".*?\\}",
                    "", playerPage) + ";";

            if (!actualMethod.contains("$") && actualMethod.matches(".*\\$.*")) {
                String extraLetter = actualMethod.split("\\$")[1].split("\\.")[0];
                if (debug)
                    System.out.println("Extra letter is " + extraLetter);
                String extraMethod = regBetween("\\$" + extraLetter + "=",
                        ".*?", "\\};", playerPage);
                if (debug)
                    System.out.println("Extra method is " + extraMethod);
                actualMethod = "var $" + extraLetter + "=" + extraMethod + "};" + actualMethod;
            }
